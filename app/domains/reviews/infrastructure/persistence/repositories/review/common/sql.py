"""Common SQL repository implementation for Review.
...
from app.domains.reviews.infrastructure.persistence.models import ReviewModel
from app.shared.domain.constants import AUDIT_FIELDS_EXCLUDE


class SQLReviewRepository:
    """Common SQL implementation for Review repository.

    This repository provides async CRUD operations for Review entities using
    ...
    async def create(
        ...
        """
        # Create Review entity (generates ULID automatically)
        # Exclude audit fields to prevent conflicts
        review = Review(
            **review_data.model_dump(exclude=AUDIT_FIELDS_EXCLUDE),
    ...
    async def find_with_count(
        ...
        Example:
            >>> reviews, total = await repo.find_with_count()
            >>> reviews, total = await repo.find_with_count({"user_id": "xxx"})
            >>> reviews, total = await repo.find_with_count(
            ...     {"entity_type": "restaurant", "status": "approved"},
            ...     offset=0,
            ...     limit=20,
            ... )
        """
